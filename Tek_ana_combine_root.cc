/////////////////////////////////////////////////////////////////////////////////////////
// To compile : g++ ana_combine_root.cc -o ana_combine_root
// -I${ROOTSYS}/include/ `root-config --cflags --libs` `python3.6m-config
// --cflags --ldflags` Requires an install of python 3.6, point to your location
// for python3.6m-config To execute (help infomation gives detail utility) :
// ./DDC10_data_readout -h
/* Revision log :
 *
...
int main(int argc, char *argv[]) {
  ...
  TString infiles(filename);
  TObjArray *inlist = infiles.Tokenize(",");
  ...
  TNtuple *pulse = new TNtuple(
      "pulse", "pulse",
      "pulseHeight:pulseRightEdge:pulseLeftEdge:pulseCharge:pulsePeakTime:"
      "CalibratedTime:baselinerms:windowratio:Run:sweep:bigstep:pulseLength5:"
  ...
  // TNtuple *event = new
  // TNtuple("event","event","charge:charge_frac:baseline:rms:npulses");

  //}
  std::cout << "creating output ntuple" << std::endl;
  // variables
  float pulseHeight = 0, pulseRightEdge = 0, pulseLeftEdge = 0, pulseCharge = 0,
        pulsePeakTime = 0, CalibratedTime = 0, windowRatio = 0, baselinerms = 0,
        sweep = 0, bigstep = 0, plen5 = 0, plen25 = 0, plen50 = 0, plen75 = 0,
        plen80 = 0, plen90 = 0, plen95 = 0, plen99 = 0;
  float charge = 0, charge_frac = 0, baseline = 0, rms = 0, npeaks = 0,
        firstTime = 0, triggerpulseHeight = 0, triggerpulseWidth = 0,
        triggerpulsePeakTime = 0, mycharge_fracj = 0, eventrate = 0;
  ...
  vector<TString> dark_count_labels;
  ...
  for (int i = 0; i < inlist->GetEntries(); i++) {
    ...
    TString thisfilename = ((TObjString *)(inlist->At(i)))->String();
    sprintf(root_file_name, "%s/%s", working_dir.c_str(), thisfilename.Data());
    ...
    dark_count.push_back(dark_hit->GetMean() / 2e-6);
    dark_count_error.push_back(dark_hit->GetMeanError() / 2e-6);
    TString thislabel =
        ((TObjString *)(((TObjArray *)thisfilename.Tokenize("."))->First()))
            ->String();
    cout << "Calculating for " << thislabel << endl;
    dark_count_labels.push_back(thislabel);
    if (use_temp) {
    ...
    for (int j = 0; j < nument; j++) {
      tree->GetEntry(j);
      float inmount[] = {pulseHeight, pulseRightEdge, pulseLeftEdge,
                         pulseCharge, pulsePeakTime,  CalibratedTime,
                         baselinerms, windowRatio,    i,
                         sweep,       bigstep,        plen5,
                         plen25,      plen50,         plen75,
                         plen80,      plen90,         plen95};
      float *inmountpoint = &inmount[0];
      pulse->Fill(inmountpoint);
      if (use_frac && pulsePeakTime > frac_start &&
          pulsePeakTime < (frac_start + frac_time))
        mycharge_frac[sweep] += pulseCharge;
      // delete inmountpoint;
      // cout<<" This is root file : "<<i<<" we are reading entry : "<<j<<" with
    ...
    if (event_tree_enable) {
      ...
      event_tree->SetBranchAddress("rate", &eventrate);
  ...
  TH1F *hi = new TH1F("h", "test", dark_count.size(), 0, dark_count.size());
  int backcount = 0;
  double maxdrate = 0;
  for (int h = 0; h < dark_count.size(); h++) {
    ...
    if (temp_dark_rate + temp_dark_rate_error > maxdrate)
      maxdrate = temp_dark_rate + temp_dark_rate_error;
    dark_plot->SetPoint(h - backcount, h, temp_dark_rate);
    dark_plot->SetPointError(h - backcount, 0, temp_dark_rate_error);
    hi->GetXaxis()->SetBinLabel(h, dark_count_labels[h].Data());
    // dark_plot->GetXaxis()->SetBinLabel(h, dark_count_labels[h].Data());
  }
  ...
  dark_plot->SetTitle(";Run ;Dark Rate (Hz)");
  TCanvas *cdark = new TCanvas("cdark", "cdark");
  dark_plot->SetMarkerStyle(24);
  dark_plot->SetMarkerColor(2);
  hi->SetMaximum(maxdrate * 1.05);
  hi->Draw();
  dark_plot->Draw("P");
  TString outpicname = working_dir + "AllDarkRates.png";
  cdark->SaveAs(outpicname.Data());
