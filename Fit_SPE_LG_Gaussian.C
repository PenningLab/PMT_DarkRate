...
int Fit_SPE_LG_Gaussian(TString infile)
{
	// Read in the file
	...
	TH1D* hspe = new TH1D("hspe", "hspe", 800, -10, 25);
	hspe->Sumw2();
	// pC/V = 5
	...
	RooRealVar q("q", "q", -6, 25);
	// RooRealVar q1("q1","q1",0.5,4.0);
	RooRealVar mean("mean", "mean", 0, -1, 1);
	RooRealVar sigma("sigma", "sigma", 1.4, 0.0, 2);

	RooRealVar mean1("mean1", "mean1", 6.5, 3, 9);
	RooRealVar sigma1("sigma1", "sigma1", 2.3, 0.0, 3);

	RooRealVar mean2("mean2", "mean2", 15, 11, 18);
	RooRealVar sigma2("sigma2", "sigma2", 3, 0.0, 6);
	...
	RooRealVar gau_frac("gau_frac", "gau_frac", 0.6, 0.0, 1.0);
	RooRealVar gau1_frac("gau1_frac", "gau1_frac", 0.2, 0.0, 1.0);
